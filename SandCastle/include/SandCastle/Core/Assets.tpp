...
namespace SandCastle
{
	...
	inline static Sprite* Assets::Get(const String& name)
	{
		...
		ASSERT_LOG_ERROR((it != Instance()->m_sprites.end()), "Sprite {0}, doesn't exists.", name);
		return &it->second;
	}
	template <>
	inline static Texture* Assets::Get(const String& name)
	{
		...
		ASSERT_LOG_ERROR((it != Instance()->m_textures.end()), "Texture {0}, doesn't exists.", name);
		return &it->second;
	}
	template <>
	inline static Shader* Assets::Get(const String& name)
	{
		...
		ASSERT_LOG_ERROR((it != Instance()->m_shaders.end()), "Shader {0}, doesn't exists.", name);
		return &it->second;
	}
	template <>
	inline static Animation* Assets::Get(const String& name)
	{
		...
		ASSERT_LOG_ERROR((it != Instance()->m_animations.end()), "Animation {0}, doesn't exists.", name);
		return &it->second;
	}
	template <>
