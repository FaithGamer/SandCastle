...
The whole chain is dot-style; `operator->` and `operator*` exist for read access to the component, but every fluent setter (`UseSprite`, `Trajectory`, `Curve`, `Distance`, `Angle`, `Lifetime`, `Scale`, `Spin`, `Fade`, `Tint`, `EasingFn`, `SpawnArea`/`NoSpawnArea`, `OnParticleDestroy`, `BurstRate`, `CountPerBurst`, `Bursts`, `Play`/`Pause`, `BurstOnce`, `DestroyWhenDone`) is mirrored on `EmitterHandle` and returns `EmitterHandle&`.

| Group | Setters | Notes |
|---|---|---|
| Visual | `UseSprite(Sprite*)` / `UseSprite({s0, s1, ...})` / `UseSprite(std::vector<Sprite*>)`, `OnLayer(LayerID)`, `WithMaterial(MaterialID)` | Falls back to `ParticleSystem::GetDefaultSprite()` / `SpriteRender::defaultLayer` / `0` if unset. List form: uniform random sprite per particle — supersedes the single sprite; entries must be non-null. |
...
| Travel | `Distance(min,max)` / `Distance(v)`, `Angle(min,max)` / `Angle(v)`, `Lifetime(min,max)` / `Lifetime(v)`, `Scale(min,max)` / `Scale(v)`, `Spin(min,max)` / `Spin(v)`, `SpinBothSides(bool)` | Angle is in degrees, `0°` points up, clockwise. Spin is degrees/second sampled per particle; with `SpinBothSides` (default true) the sample is randomly sign-flipped, so `Spin(720, 1080)` spins fast in both directions with no slow spinners. Non-zero spin also randomizes the initial angle in `[0, 360)`. |
...
| Events | `OnParticleDestroy(std::function<void(const ParticleSignal&)>)` | Fires once per particle with a `ParticleSignal` (`.position` = final world position; struct so it can grow without breaking the signature), just before it dies at end of life. The functor is copied once into a ref-counted runtime table in `ParticleSystem` (not per particle — particles hold a small id), so it keeps firing for in-flight particles even after the emitter is destroyed, and is freed when the last referencing particle dies. Also works with the temporary-spec `Burst(pos, spec)` overload. |
