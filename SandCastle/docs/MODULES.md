...
| [Time.h](../include/SandCastle/Core/Time.h) | `Time`, `Clock`, `Time::Delta()/FixedDelta()/UnscaledDelta()/UnscaledFixedDelta()` | `Time` implicitly converts to seconds (float). Built on `std::chrono::microseconds`. |
...
| [Particle.h](../include/SandCastle/Render/Particle.h) | `Particle` (component), `ParticleEasingFn` | t goes 0→1 along a `Beziers` curve. |
| [ParticleSystem.h](../include/SandCastle/Render/ParticleSystem.h) | `ParticleSystem` (system), `ParticleTraj` (Straight/CubicIn/CubicOut/CubicInOut) | `Make(p1, p2, ...)` returns the spawned `Entity`. Auto-pushed. |
...
| [Inputs.h](../include/SandCastle/Input/Inputs.h) | `Inputs` (singleton), `InputMapContainer`, `Inputs::PeripheralFlag` (Mouse/Keyboard/Gamepad) | `CreateInputMap("name")`, `Get(map, input)`, rebinding workflow (`StartRebind`/`EndRebind`/`AddForbiddenBinding`). `controllerUsedLast` tracks last-used peripheral. |
